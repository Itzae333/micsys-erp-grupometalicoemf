...
interface SolicitudAbonoPublica {
  valido: boolean;
...
export default function AprobarEdicionAbonoPage() {
  const { token } = useParams<{ token: string }>();

  const [data, setData] = useState<SolicitudAbonoPublica | null>(null);
  ...
  useEffect(() => {
    api.get<SolicitudAbonoPublica>(`/solicitudes-abono/token/${token}`, { skipAuth: true })
      .then((res) => {
        setData(res);
  ...
  async function onAprobar() {
    if (!aprobadorId) {
      setError('Selecciona quién autoriza el cambio.');
      return;
    }
    setError(null);
    setProcesando('aprobar');
    try {
      await api.post(`/solicitudes-abono/token/${token}/aprobar`, { aprobador_usuario_id: aprobadorId }, { skipAuth: true });
  ...
  async function onRechazar() {
    ...
    try {
      await api.post(`/solicitudes-abono/token/${token}/rechazar`, {
      ...
      <div className="bg-white border border-steel-200 rounded-xl p-6 shadow-sm">
        ...
        {!loading && data && resultado === 'aprobada' && (
          <div className="text-center py-4">
            ...
            <p className="text-body font-semibold text-steel-900">Cambio autorizado</p>
            <p className="text-body-sm text-steel-500 mt-1">
              El abono de la nota #{String(data.folio).padStart(4, '0')} ya fue {data.accion === 'ELIMINAR' ? 'eliminado' : 'actualizado'}.
            </p>
          </div>
        )}
        ...
        {!loading && data && !resultado && (
          <>
            ...
            <p className="text-body-sm text-steel-500 mb-4">
              {data.solicitante ? `${data.solicitante.nombre} ${data.solicitante.apellidos}` : 'Un usuario'} solicitó{' '}
              {data.accion === 'ELIMINAR' ? 'eliminar' : 'editar'} un abono de esta venta.
            </p>

            <div className="bg-steel-50 rounded-lg px-3 py-2.5 mb-4 space-y-1">
              <div className="flex items-center justify-between text-body-sm">
                <span className="text-steel-500">Abono actual</span>
                <span className="font-semibold text-steel-900">
                  {formatPrecio(data.monto_actual)} — {METODO_LABEL[data.metodo_actual] ?? data.metodo_actual}
                </span>
              </div>
              {data.accion === 'EDITAR' && (
                <div className="flex items-center justify-between text-body-sm">
                  <span className="text-steel-500">Nuevo valor</span>
                  <span className="font-semibold text-emerald-700">
                    {data.nuevo_monto !== null ? formatPrecio(data.nuevo_monto) : ''} — {data.nuevo_metodo ? (METODO_LABEL[data.nuevo_metodo] ?? data.nuevo_metodo) : ''}
                  </span>
                </div>
              )}
